/*
 * Matrix multiplication using memory allocated in user space
 */
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
...
int main(int argc, char *argv[]) {
    ...
    a = malloc(M_SIZE*M_SIZE*sizeof(float));
    b = malloc(M_SIZE*M_SIZE*sizeof(float));
    c = malloc(M_SIZE*M_SIZE*sizeof(float));
    result = malloc(M_SIZE*M_SIZE*sizeof(float));
    ...
    status = xdmaSubmitBuffer(a, M_SIZE*M_SIZE*sizeof(float), XDMA_ASYNC, dev, inChannel, &aTrans);
        checkError(status, "Error submitting A matrix\n");
    status = xdmaSubmitBuffer(b, M_SIZE*M_SIZE*sizeof(float), XDMA_ASYNC, dev, inChannel, &bTrans);
        checkError(status, "Error submitting B matrix\n");
    status = xdmaSubmitBuffer(c, M_SIZE*M_SIZE*sizeof(float), XDMA_ASYNC, dev, inChannel, &cTrans);
        checkError(status, "Error submitting C matrix\n");
    status = xdmaSubmitBuffer(result, M_SIZE*M_SIZE*sizeof(float), XDMA_ASYNC, dev, outChannel, &outTrans);
